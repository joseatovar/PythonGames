# program template for Spaceship - RiceRocks
import simplegui
import math
import random
...
def process_sprite_group(set_group, canvas):
    group_copy = set(set_group)
    for sprite in group_copy:
        sprite.draw(canvas)
        if sprite.update(): # update and see lifespan
...
def group_collide(group, other_object):
    ...
    for sprite in group_copy:
        if sprite.collide(other_object):
            group.remove(sprite)
            number_coll += 1
            explosion_group.add(Sprite(sprite.pos, other_object.vel, 0, 0,explosion_image, explosion_info, explosion_sound)) #add explosion
    return number_coll
...
class Sprite:
    ...
    def draw(self, canvas):
        if self.animated:
            time = self.age
            canvas.draw_image(self.image, [self.image_center[0] + time * self.image_size[0], self.image_center[1]], self.image_size,self.pos, self.image_size, self.angle)
        else:
            canvas.draw_image(self.image, self.image_center, self.image_size,self.pos, self.image_size, self.angle)
...
def draw(canvas):
    ...
    if lives <= 0 : #restart music and started variable
        started = False
        soundtrack.rewind()
...
def rock_spawner():
    ...
    if started and len(rock_group) < 12:
        ...
        if dist(rock_pos, my_ship.get_position()) >= my_ship.get_radius() * 3:
            # score/10 is a value to make more difficult the game
            rock_vel = [random.random() * (.6 + score/10) - .3, random.random()* (.6 + score/10) - .3]
