...
# art assets created by Kim Lathrop, may be freely re-used in non-commercialprojects, please credit Kim
...
class Ship:
    ...
    def update(self):
        #update velocity
        if self.thrust: #acceleration
            acc = 0.07
            velocity = angle_to_vector(self.angle)
            for i in range(2):
                self.vel[i] += velocity[i] * acc # acceleration
        else: #friction
            fric = 0.98
            for i in range(2):
                self.vel[i] = self.vel[i] * fric
                
        #update position of the ship
        self.pos[0] = (self.pos[0] + self.vel[0]) % WIDTH
        self.pos[1] = (self.pos[1] + self.vel[1]) % HEIGHT
        #update angle of the ship
    ...
    def active_thrusters(self,option):
        self.thrust = option
        if self.thrust:
    ...
    def shoot(self):
        ...
        pos = [self.pos[0] + (vector[0]*self.image_center[0]), self.pos[1] + (vector[1]*self.image_center[0])]
        vel = [self.vel[0] + vector[0] * 10,self.vel[1] + vector[1] * 10]
...
def draw(canvas):
    ...
    #lives
    canvas.draw_text("Lives", (60, 60), 30, "White")
    canvas.draw_text(str(lives), (80, 90), 30, "White")
    #score
    canvas.draw_text("Score", (WIDTH - 140, 60), 30, "White")
    canvas.draw_text(str(score), (WIDTH - 120, 90), 30, "White")
...
def rock_spawner():
    ...
    vel = angle_to_vector(angle)
    angle_vel = (random.random() / 9 ) * random.choice([-1,1])
    a_rock = Sprite(pos, vel, angle, angle_vel, asteroid_image, asteroid_info)

# key handlers
def keydown(key):
    if key == simplegui.KEY_MAP['up']:
        my_ship.active_thrusters(True)
...
def keyup(key):
    if key == simplegui.KEY_MAP['up']:
        my_ship.active_thrusters(False)
    elif key == simplegui.KEY_MAP['left']:
        my_ship.increment_angular_vel()
    elif key == simplegui.KEY_MAP['right']:
        my_ship.decrement_angular_vel()
